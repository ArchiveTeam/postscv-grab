...
LUA_SHA1 = get_hash(os.path.join(CWD, 'postscv.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'user':
                wget_args.extend(['--warc-header', 'postscv-user: '+item_value])
                wget_args.append('https://posts.cv/'+item_value)
            elif item_type == 'post':
                user, post = item_value.split(':')
                wget_args.extend(['--warc-header', 'postscv-post: '+post])
                wget_args.append('https://posts.cv/{}/{}'.format(user, post))
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/index.php/File:Postscv-icon.png" height="50px" title=""/>
        <h2>Posts.cv <span class="links"><a href="https://posts.cv/">Website</a> &middot; <a href="http://tracker.archiveteam.org/postscv/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Posts.cv">Wiki</a></span></h2>
        <p>Archiving Posts.cv.</p>
    '''
)
